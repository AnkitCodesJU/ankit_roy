...
export default function ProjectModal({ project, onClose }) {
  ...
  const descRef = useScrollReveal({ direction: "up", distance: 30, threshold: 0.1 });
  const techRef = useScrollReveal({ selector: ".tech-icon-item", stagger: 50, distance: 20 });
  ...
  const renderDescription = (desc) => {
      ...
      return (
        <ul className="space-y-6">
          {desc.map((point, index) => (
            <li key={index} className="flex gap-4 text-[#eee] text-lg md:text-xl leading-relaxed font-sans">
              <span className="text-blood mt-2 shrink-0 text-2xl">•</span>
              <span dangerouslySetInnerHTML={{ 
                __html: point.replace(/\*\*(.*?)\*\*/g, '<strong class="text-blood font-bold tracking-wide">$1</strong>') 
              }} />
            </li>
          ))}
        </ul>
    ...
    return <p className="text-[#eee] text-xl leading-relaxed font-sans">{desc}</p>;
  };
      ...
      >
        ...
        <div className="min-h-[100%] md:min-h-[90vh] flex flex-col shrink-0 relative">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-40 animate-bounce">
            <span className="text-[10px] font-mono tracking-[4px] uppercase text-dim">Scroll Detail</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-10">
              {project.techIcons ? (
                project.techIcons.map((icon, i) => (
                  <div key={i} className="tech-icon-item flex flex-col items-center gap-4 group/icon">
